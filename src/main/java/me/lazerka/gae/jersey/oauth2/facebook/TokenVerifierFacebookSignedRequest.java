...
//					.queryParam("redirect_uri={redirect-uri}");
	...
	protected static final UriBuilder userEndpoint =
			UriBuilder.fromUri("https://graph.facebook.com/v2.6/user/{userId}")
	...
	@Override
	public UserPrincipal verify(String signedRequestToken) throws IOException, InvalidKeyException {
	...
	protected String exchangeCodeForAppAccessToken(String code) throws IOException, InvalidKeyException {
		...
		checkState(content.startsWith(prefix), "/access_token response doesn't start with " + prefix);
